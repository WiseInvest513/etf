// utils/lazy-data.js — 15个懒人组合（与 LazyPage.jsx 保持同步）
const YEARS = [2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024, 2025];

const ETF_LABELS = {
  VTI: '美国股票(VTI)', TLT: '长期国债(TLT)', IEF: '中期国债(IEF)',
  GSG: '大宗商品(GSG)', GLD: '黄金(GLD)',       BIL: '短期美债(BIL)',
  BND: '美债综合(BND)', VNQ: '房地产(VNQ)',       VXUS:'国际股票(VXUS)',
  SPY: '标普500(SPY)',  SHY: '短期债(SHY)',       TIP: '通胀债(TIP)',
  EFA: '发达市场(EFA)', VWO: '新兴市场(VWO)',      VB:  '小盘股(VB)',
  VBR: '小盘价值(VBR)', IVE: '大盘价值(IVE)',
};
...
module.exports = [
  {
    ...
    author: 'Ray Dalio',
    color: '#6366f1',
    desc: '桥水基金创始人的核心理念：为各种经济周期等权分配风险，追求平稳穿越牛熊。',
    longDesc: [
  ...
  },
  {
    ...
    author: 'Harry Browne',
    color: '#f59e0b',
    desc: '四分天下：股票应对繁荣，黄金应对通胀，长债应对通缩，现金应对衰退。极致简洁。',
    longDesc: [
      '永久组合由美国投资作家 Harry Browne 于1981年创立，设计理念极其简洁：把财富平均分成四份，分别配置股票（25%）、黄金（25%）、长期国债（25%）和现金/短期国债（25%），以应对经济可能处于的四种状态：繁荣、通胀、通缩和萧条。',
      '这四类资产通常具有低相关甚至负相关性：股市上涨时黄金往往横盘，衰退时债券和黄金齐涨，通胀飙升时黄金大涨而债券受损。正是这种天然对冲使永久组合成为所有懒人组合中回撤最小的之一（-15.93%）。代价是长牛期间收益落后于纯股票组合。',
      '2022 年黄金的强势和短期国债利息收入帮助组合在熊市中仅下跌 -12.86%，远优于标普500的 -18%。2025 年黄金大涨 63.68%，永久组合全年录得 +22.50%，是本站所有组合中 2025 年表现最佳的之一。适合追求绝对稳健、风险极度厌恶的投资者。',
    ],
  ...
  },
  {
    ...
    author: '传统智慧',
    color: '#3b82f6',
    desc: '最经典的资产配置组合，六成股票获取增长，四成债券降低波动。几十年来机构投资者的默认选择。',
    longDesc: [
      '60/40 组合是全球最广为人知的资产配置策略，简单到不需要任何金融学背景就能理解和执行：60% 投资于美国股票指数基金，40% 投资于美国综合债券指数基金。这一比例在过去半个世纪被无数养老金、保险资金和个人投资者奉为标准。',
      '其逻辑在于：股票提供长期增长引擎，债券在熊市中起到「压舱石」作用，两者历史上大多数时期呈负相关。当股市崩盘时，投资者涌入国债避险，债券价格上涨可以对冲股票亏损，平滑整体净值曲线。然而 2022 年是个例外——股债双杀导致该组合下跌 -17.77%。',
  ...
  },
  {
    ...
    color: '#0284c7',
    desc: '耶鲁大学CIO David Swensen为个人投资者设计的简化版，加入REITs和TIPS，注重真实资产暴露。',
    longDesc: [
  ...
  },
  {
    ...
    author: 'Meb Faber',
    color: '#10b981',
    desc: '仿照哈佛/耶鲁等顶尖捐赠基金构建，五类资产等权分配，含大宗商品，周期性轮动效果明显。',
    longDesc: [
      ...
      '2017—2025 年化收益 8.17%，最大回撤 -21.97%，夏普比率 0.53。大宗商品的高波动性使其在部分年份拖累表现（如 2020 年 GSG 下跌 -23.92%），但长期而言为组合提供了额外的分散化收益来源。',
    ],
  ...
  },
  {
    ...
    color: '#84cc16',
    desc: 'Bill Schultheis提倡：做好资产配置后，去喝杯咖啡不要管它。7类资产广泛分散，债券压舱稳定。',
    longDesc: [
      ...
      '2017—2025 年化收益 6.64%，最大回撤 -19.70%，夏普比率 0.43。由于 40% 的高债券占比，组合整体偏保守，在股票牛市期间表现落后于纯股票组合，但在2018、2022年的调整中显示出较强的抗跌性。适合保守型投资者作为核心长期持仓。',
    ],
  ...
  },
  {
    ...
    color: '#06b6d4',
    desc: 'Bernstein的极简主义：四类等权资产，完全不需要动脑筋，却能获得市场平均回报。',
    longDesc: [
      ...
      '2017—2025 年化收益 8.83%，最大回撤 -19.69%，夏普比率 0.57。在所有15个组合中属于中等偏上水平，同时执行难度极低，非常适合初次接触指数投资的新手，以及没有精力深入研究市场的忙碌上班族。',
    ],
  ...
  },
  {
    ...
    author: 'Rick Ferri',
    color: '#f97316',
    desc: '指数基金专家Rick Ferri的精华：四个基金覆盖全球股票+债券+REITs，低费率、广分散。',
    longDesc: [
      ...
      '2017—2025 年化收益 9.60%，是所有组合中仅次于巴菲特90/10的最高年化收益，最大回撤 -23.55%，夏普比率 0.60。对于希望获得接近全股票收益、同时保持基础风险管理的投资者，核心四基是非常均衡的选择。',
    ],
  ...
  },
  {
    ...
    color: '#ef4444',
    desc: '巴菲特在遗嘱中建议：90%投入标普500指数基金，10%短期国债。本站CAGR最高组合。',
    longDesc: [
      '这是所有懒人组合中最著名的一个。Warren Buffett 在2013年致伯克希尔股东信中披露，他在遗嘱中为妻子的信托基金留下了明确的投资指示：90%买入低费率标普500指数基金，10%买入短期政府债券。理由是：长期而言没有主动基金经理能持续跑赢指数，最好的做法就是拥有美国商业的一小份，等待美国经济增长。',
      ...
      '2017—2025 年化收益高达 12.41%，在所有15个组合中排名第一，但波动率也是最高的（14.33%）。这一组合的隐含前提是：投资者有足够长的投资期限（15年以上），有足够强的心理承受能力在市场大跌时坚持持有，不会因为短期浮亏而割肉离场。',
    ],
  ...
  },
  {
    ...
    author: 'David Swensen',
    color: '#a855f7',
    desc: '耶鲁CIO面向普通个人投资者的另一版本：纳入新兴市场和TIPS，追求更广泛的全球多元化。',
    longDesc: [
      ...
      '2017—2025 年化收益 7.70%，最大回撤 -21.37%，夏普比率 0.51，波动率 10.98%。新兴市场在 2017 年贡献了较好的收益（VWO +26.49%），但在 2018、2022 年则拖累组合。对比耶鲁捐赠版，两者指标几乎相同，风险收益特征高度相似。',
    ],
    cagr: 7.70, maxDrawdown: -21.37, sharpe: 0.51, sortino: 0.73, volatility: 10.98,
  ...
  },
  {
    ...
    color: '#1d4ed8',
    desc: '先锋基金创始人 Bogle 的终极哲学：一只美国股票 + 一只国际股票 + 一只债券，覆盖全球市场，极低费率。',
    longDesc: [
      ...
      '如今 Bogleheads 社区拥有数十万名来自全球的个人投资者，三基金组合是这个社区最受推崇的投资方式。典型的分配建议是根据投资者年龄调整债券比例（如：年龄=债券占比），本次回测采用 50% VTI + 30% VXUS + 20% BND 的中等股债比例。',
      '2017—2025 年化收益 10.47%，在所有 15 个组合中位居第二（仅次于巴菲特90/10），最大回撤 -23.18%，夏普比率 0.70。高达 80% 的全球股权配置带来了较好的长期收益，尤其 2025 年表现亮眼达 +19.67%。对于认同指数化投资、希望低成本实现全球分散的投资者，三基金是目前已知的最简洁且高效的方案之一。',
    ],
  ...
  },
  {
    ...
    color: '#b45309',
    desc: '在永久组合基础上引入小盘价值股，五类资产等权。近百年验证最大回撤仅 -17.79%，攻守兼备。',
    longDesc: [
  ...
  },
  {
    ...
    color: '#0e7490',
    desc: '按全球资产市值比例配置股票、债券、房地产，不做主观判断，最大程度代表全球财富分布。',
    longDesc: [
      '全球市场组合（GMP）的理论基础来自现代投资组合理论（MPT）的一个重要推论：在有效市场中，所有投资者共同持有的资产组合，即为最优切点组合。换言之，如果把全球所有可投资资产的市值加总，并按比例持有，就理论上获得了与全球所有投资者平均相同的风险收益。',
      '本组合近似全球市值分布：30% VTI 美国股票、25% VXUS 国际股票、25% BND 全球债券（债券占全球可投资资产约50%）、10% VNQ 全球房地产信托、10% GLD 实物资产/黄金（作为商品类别代理）。各类资产权重大致反映其在全球财富中的比例。',
  ...
  },
  {
    ...
    color: '#6d28d9',
    desc: '源自2000年前的犹太智慧，四类资产各25%：美股、国际股、债券、房地产。分散效果极佳。',
    longDesc: [
      '《塔木德》是犹太教最重要的经典之一，其中包含一段流传2000多年的理财智慧：「一个人应当将其资产三等分：三分之一用于土地，三分之一用于商业，三分之一留存在手」——大致对应现代的房地产、股票和现金。资产配置先驱 Roger Gibson 在《资产配置》一书中将这一古老哲学更新为四类资产各25%的现代版本。',
  ...
  },
  {
    ...
    color: '#047857',
    desc: '因子投资先驱 Merriman 的代表作：11类资产广泛覆盖全球，叠加价值和小盘因子，适合长期持有者。',
    longDesc: [
      ...
      '2017—2025 年化收益 9.03%，最大回撤 -27.54%（本站最高），年化波动率 15.42%（本站最高），夏普比率 0.52。高波动和深回撤是持有这个组合必须接受的代价。2021 年小盘股大涨（VBR +28.77%）推动组合大涨 +19.37%，2025 年再次录得 +22.62%。适合对因子投资理论有深入认识、投资期限超过20年的进取型投资者。',
    ],
...
module.exports.ETF_LABELS = ETF_LABELS;
module.exports.PIE_COLORS = PIE_COLORS;
